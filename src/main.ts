...
const SIZES = {
  ...
  FAN: { baseHeight: 1.5, bladeLength: 1.2 },
  AXES_HELPER: 5,
} as const;
...
interface GameObject {
  mesh: THREE.Mesh;
  body: CANNON.Body;
  type: 'ball' | 'domino' | 'ramp' | 'box' | 'seesaw' | 'mountain' | 'shrine' | 'road' | 'plank' | 'guardrail' | 'penguin' | 'cow' | 'stairs' | 'train' | 'trampoline' | 'goal' | 'fan';
  ...
  patrolDirection?: { x: number; z: number }; // 動物のパトロール方向
  patrolTimer?: number; // 動物のパトロールタイマー
  isEating?: boolean; // 牛が草を食べているか
  eatingTimer?: number; // 牛が草を食べるタイマー
  ...
  fanRange?: number; // 扇風機の風が届く範囲
  fanForce?: number; // 扇風機の風力
  fanDirection?: THREE.Vector3; // 扇風機の風向き
  fanBlades?: THREE.Mesh; // 扇風機の羽根（回転用）
}
...
function createBox(position: { x: number; y: number; z: number }): GameObject {
  ...
  body.material = new CANNON.Material({ friction: 0.3, restitution: 0.5 });
...
function createStairs(
  ...
  dummyMesh.position.set(0, 0, 0); // グループの中心（0,0,0）に配置
  dummyMesh.userData.isDummy = true; // ダミーメッシュであることをマーク
  group.add(dummyMesh);
...
function createGrassZone(
  ...
  grassPlane.position.set(position.x, position.y + 0.05, position.z);
...
// 扇風機の作成
function createFan(position: { x: number; y: number; z: number }): GameObject {
  const group = new THREE.Group();

  // 土台
  const baseGeometry = new THREE.CylinderGeometry(0.2, 0.4, SIZES.FAN.baseHeight, 16);
  const baseMaterial = new THREE.MeshStandardMaterial({
    color: COLORS.FAN_BASE,
    metalness: 0.5,
    roughness: 0.5
  });
  const base = new THREE.Mesh(baseGeometry, baseMaterial);
  base.position.y = SIZES.FAN.baseHeight / 2;
  group.add(base);

  // モーター部分
  const motorGeometry = new THREE.CylinderGeometry(0.3, 0.3, 0.4, 16);
  const motor = new THREE.Mesh(motorGeometry, baseMaterial);
  motor.rotation.x = Math.PI / 2;
  motor.position.y = SIZES.FAN.baseHeight;
  group.add(motor);

  // 羽根（回転部分）
  const bladesGroup = new THREE.Group();
  const bladeGeometry = new THREE.BoxGeometry(0.1, SIZES.FAN.bladeLength, 0.05);
  const bladeMaterial = new THREE.MeshStandardMaterial({
    color: COLORS.FAN_BLADES,
    metalness: 0.1,
    roughness: 0.2
  });
  ...
  for (let i = 0; i < 3; i++) {
    ...
    blade.position.y = SIZES.FAN.bladeLength / 2 - 0.2;
  ...
  bladesGroup.position.set(0, SIZES.FAN.baseHeight, 0.25);
  group.add(bladesGroup);

  // 風の方向を示す矢印（半透明）
  const arrowHelper = new THREE.ArrowHelper(
    new THREE.Vector3(0, 0, 1),
    new THREE.Vector3(0, SIZES.FAN.baseHeight, 0),
    2,
    0x00ffff,
    0.5,
    0.3
  ...
  // 物理ボディ（円柱）
  const shape = new CANNON.Cylinder(0.4, 0.4, SIZES.FAN.baseHeight, 8);
  const body = new CANNON.Body({ mass: 0 }); // 固定
  // CANNONのCylinderはZ軸向きなので回転が必要
  const q = new CANNON.Quaternion();
  q.setFromAxisAngle(new CANNON.Vec3(1, 0, 0), -Math.PI / 2);
  body.addShape(shape, new CANNON.Vec3(0, SIZES.FAN.baseHeight / 2, 0), q);
  body.position.set(position.x, position.y, position.z);
  world.addBody(body);
  ...
  return {
    ...
    fanRange: 10, // 風の届く距離
    fanForce: 5,  // 風の強さ
    fanDirection: new THREE.Vector3(0, 0, 1), // 初期方向（Z軸正方向）
    fanBlades: bladesGroup as any
  };
}

// 花火エフェクトの作成（打ち上げ花火風）
function createFireworks(position: { x: number; y: number; z: number }): void {
  const colors = [0xffd700, 0xff0000, 0x00ff00, 0x0000ff, 0xffff00, 0xff00ff, 0x00ffff, 0xff8c00];
  const numLaunches = 6; // 同時に打ち上げる花火の数

  for (let i = 0; i < numLaunches; i++) {
    // 各花火の色をランダムに選択（金色を多めに）
    const explosionColor = i < 2 ? 0xffd700 : colors[Math.floor(Math.random() * colors.length)];

    // 打ち上げ位置を少しずらす（円形に配置）
    const angle = (i / numLaunches) * Math.PI * 2;
    const radius = 0.5 + Math.random() * 0.5;
    const launchX = position.x + Math.cos(angle) * radius;
    const launchZ = position.z + Math.sin(angle) * radius;

    // 打ち上げパーティクル（上に打ち上がる）
    const launchGeometry = new THREE.SphereGeometry(0.12, 8, 8);
    ...
    // 打ち上げ速度（上方向に速く）
    const launchSpeed = 10 + Math.random() * 4; // 10-14の範囲
    launchParticle.position.set(launchX, position.y, launchZ);
    launchParticle.userData.velocity = new THREE.Vector3(0, launchSpeed, 0);
    launchParticle.userData.gravity = -0.2;
    launchParticle.userData.trailParticles = []; // 軌跡用のパーティクル

    // 爆発する高さ（ランダムに10-20m、高めに）
    const targetHeight = position.y + Math.random() * 10 + 10;
...
function explodeFireworks(position: { x: number; y: number; z: number }, color: number): void {
  const numParticles = 400; // 爆発時のパーティクル数をさらに増やす

  for (let i = 0; i < numParticles; i++) {
    // パーティクルのサイズをランダムに（大きいものと小さいものを混ぜる）
    const size = Math.random() < 0.3 ? 0.2 : 0.12; // 30%が大きめ
    const particleGeometry = new THREE.SphereGeometry(size, 8, 8);
    const particleMaterial = new THREE.MeshStandardMaterial({
      color: color,
      emissive: color,
      emissiveIntensity: 2.0, // より明るく
      transparent: true,
      opacity: 1.0
    });
    ...
    const speed = Math.random() * 15 + 10; // 爆発速度を上げる（10-25）
    ...
    particle.userData.life = 500; // 約8.3秒間（500フレーム）
    particle.userData.maxLife = 500;
...
function createPenguin(position: { x: number; y: number; z: number }): GameObject {
  ...
  const bodyMaterial = new THREE.MeshStandardMaterial({
    color: 0xd3d3d3, // ライトグレー
    roughness: 0.9,  // ふわふわ感
    metalness: 0.0
  });
  const body = new THREE.Mesh(bodyGeometry, bodyMaterial);
  body.scale.set(1, 1.1, 1); // 少し縦長に
  body.position.y = 0.4;
  group.add(body);
  ...
  // 顔の白い部分（頭の手前に配置してマスクのように見せる）
  const faceGeometry = new THREE.SphereGeometry(0.28, 32, 32);
  const faceMaterial = new THREE.MeshStandardMaterial({
    color: 0xffffff,
    roughness: 0.9
  });
  const face = new THREE.Mesh(faceGeometry, faceMaterial);
  face.position.set(0, 0.85, 0.08); // 頭より少し手前
  face.scale.set(0.9, 0.8, 0.9); // 少し平たく
  group.add(face);

  // ペンギンのくちばし（黒、少し丸みのある円錐）
  const beakGeometry = new THREE.ConeGeometry(0.06, 0.15, 16);
  const beakMaterial = new THREE.MeshStandardMaterial({
    color: 0x111111, // 黒
    roughness: 0.4
  });
  const beak = new THREE.Mesh(beakGeometry, beakMaterial);
  beak.rotation.x = Math.PI / 2; // 前に向ける
  ...
  leftEye.position.set(-0.12, 0.9, 0.32);
  group.add(leftEye);
  const rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
  rightEye.position.set(0.12, 0.9, 0.32);
  group.add(rightEye);

  // ペンギンの翼（グレー、体と同じ色）
  const wingGeometry = new THREE.SphereGeometry(0.15, 32, 16);
  // 翼を平たく変形
  const leftWing = new THREE.Mesh(wingGeometry, bodyMaterial);
  leftWing.scale.set(0.5, 1.5, 0.8);
  leftWing.position.set(-0.38, 0.5, 0);
  leftWing.rotation.z = Math.PI / 8; // 少し開く
  group.add(leftWing);

  const rightWing = new THREE.Mesh(wingGeometry, bodyMaterial);
  rightWing.scale.set(0.5, 1.5, 0.8);
  rightWing.position.set(0.38, 0.5, 0);
  rightWing.rotation.z = -Math.PI / 8; // 少し開く
  group.add(rightWing);
  ...
  leftFoot.scale.set(1, 0.5, 1.5); // 平たく長く
  ...
  // 物理ボディ（球体と円柱の組み合わせで近似、あるいはシンプルなカプセル）
  // 重心を低くして倒れにくくする
  const radius = 0.4;
  const height = 0.8;
  const shape = new CANNON.Cylinder(radius * 0.8, radius, height, 16);

  const bodyPhy = new CANNON.Body({
    mass: 2, // 少し重くして安定させる
    fixedRotation: true // 転倒防止（起き上がりこぼしのようにしたい場合はfalseにして重心調整が必要だが、ここでは簡易的に固定）
  });

  // 転倒防止のため、回転を制限（Y軸周りの回転のみ許可する場合はこれでは不十分だが、
  // angularFactorを使うと良い。ここではシンプルに直立を維持させる）
  bodyPhy.angularFactor.set(0, 1, 0); // Y軸周りの回転のみ許可

  bodyPhy.addShape(shape);
  bodyPhy.position.set(position.x, position.y + 0.4, position.z);
  bodyPhy.material = new CANNON.Material({ friction: 0.3, restitution: 0.1 });
...
function createTrainCar(isFirstCar: boolean = false): THREE.Group {
  ...
  // 新幹線のライン（特徴的な青緑色のストライプ）
  // 上側のライン（青緑色、窓の下）
  const line1Geometry = new THREE.BoxGeometry(bodyLength, 0.2, bodyWidth + 0.1);
  const line1Material = new THREE.MeshStandardMaterial({
    color: 0x4dd0e1, // 青緑色（light blue-green）
    metalness: 0.5,
    roughness: 0.3
  });
  const line1 = new THREE.Mesh(line1Geometry, line1Material);
  line1.position.set(0, bodyHeight / 2 - 0.2, 0);
  carGroup.add(line1);

  // 下側のライン（濃い青）
  const line2Geometry = new THREE.BoxGeometry(bodyLength, 0.15, bodyWidth + 0.1);
...
function createTrain(position: { x: number; y: number; z: number }, initialAngle: number = 0): GameObject {
  ...
  car.rotation.set(0, 0, 0); // 車両グループの回転を0に固定
  group.add(car);

  // 先頭の円錐への参照を保存（回転を毎フレーム設定するため）
  let noseMesh: THREE.Mesh | null = null;
  car.traverse((child) => {
    if (child instanceof THREE.Mesh && child.geometry instanceof THREE.ConeGeometry) {
      noseMesh = child;
    }
  });
  (group as any).noseMesh = noseMesh; // 後で参照できるように保存

  group.position.set(position.x, position.y, position.z);
  group.rotation.set(0, initialAngle, 0); // X, Z軸は0に固定
  group.castShadow = true;
  group.receiveShadow = true;
  scene.add(group);
...
function checkCollision(
  type: 'domino' | 'ramp' | 'box' | 'seesaw' | 'mountain' | 'shrine' | 'road' | 'plank' | 'guardrail' | 'stairs' | 'trampoline' | 'goal' | 'fan',
  position: { x: number; y: number; z: number },
  ...
  switch (type) {
    ...
    case 'goal':
      ...
      break;
    case 'fan':
      bounds = { width: 1, height: 2, depth: 1 };
      break;
    default:
...
function findFreePosition(
  type: 'domino' | 'ramp' | 'box' | 'seesaw' | 'plank' | 'guardrail' | 'stairs' | 'shrine' | 'mountain' | 'trampoline' | 'goal' | 'fan',
  centerX: number = 0,
  centerZ: number = 0,
...
// 扇風機を追加するボタン
document.getElementById('add-fan')?.addEventListener('click', () => {
  const position = findFreePosition('fan', 0, 0, 50, 10);
  if (position) {
    const fan = createFan({ x: position.x, y: 0, z: position.z });
    gameObjects.push(fan);
  }
});
  ...
  if (selectedObject && selectedObjectOriginalMaterial) {
    if (selectedObject.type === 'stairs' && selectedObject.stairsGroup && (selectedObject as any).originalMaterials) {
      ...
      selectedObject.stairsGroup.traverse((child) => {
        if (child instanceof THREE.Mesh && !child.userData.isDummy) {
...
renderer.domElement.addEventListener('mousedown', (event) => {
  ...
  if (intersects.length > 0) {
    ...
    if (clickedObject && clickedObject.type !== 'ball') {
      ...
      dragPlane.setFromNormalAndCoplanarPoint(new THREE.Vector3(0, 1, 0), intersectionPoint);
      // 階段の場合はグループの位置を使用
  ...
  if (isDragging && selectedObject && dragPlane) {
    ...
    if (intersection) {
      ...
      if (selectedObject.type === 'stairs' && selectedObject.stairsBodies && selectedObject.stairsGroup) {
        ...
        // 基準となる物理ボディも更新しない（stairsBodies[0]なのでループ内で更新済み）
        // selectedObject.body.position.set(...) を実行すると、最初のステップがグループ中心に移動してしまうため削除
      } else {
        // 通常のオブジェクトの移動
      ...
      // 階段の場合は回転させない（グループの回転を変更すると、子要素の位置がずれるため）
      const originalQuaternion = (selectedObject as any).originalQuaternion;
      if (originalQuaternion && selectedObject.type !== 'stairs') {
...
renderer.domElement.addEventListener('mouseup', () => {
  if (isDragging && selectedObject) {
    ...
    // メッシュと物理ボディの位置を完全に同期
    // メッシュと物理ボディの位置を完全に同期
    // 階段の場合は、body（最初のステップ）とmesh（グループ中心）の位置が異なるため同期しない
    if (selectedObject.type !== 'stairs') {
      selectedObject.body.position.set(
  ...
  savedState.objects.forEach(savedObj => {
    ...
    switch (savedObj.type) {
      ...
      case 'goal':
        ...
        // 回転を復元
        obj.mesh.quaternion.set(savedObj.rotation.x, savedObj.rotation.y, savedObj.rotation.z, savedObj.rotation.w);
        obj.body.quaternion.set(savedObj.rotation.x, savedObj.rotation.y, savedObj.rotation.z, savedObj.rotation.w);
        break;
      case 'fan':
        obj = createFan({ x: pos.x, y: pos.y, z: pos.z });
        // 回転を復元
        obj.mesh.quaternion.set(savedObj.rotation.x, savedObj.rotation.y, savedObj.rotation.z, savedObj.rotation.w);
        obj.body.quaternion.set(savedObj.rotation.x, savedObj.rotation.y, savedObj.rotation.z, savedObj.rotation.w);
        break;
...
function animate() {
  ...
  gameObjects.forEach(obj => {
    ...
    // 扇風機の処理
    if (obj.type === 'fan' && obj.fanBlades && obj.fanDirection) {
      // 羽根を回転
      obj.fanBlades.rotation.z -= 0.2;

      // 風力を適用
      const fanPos = obj.body.position;
      const fanDir = new THREE.Vector3(0, 0, 1).applyQuaternion(obj.mesh.quaternion).normalize();

      gameObjects.forEach(target => {
        if (target === obj || target.body.mass === 0) return; // 自分自身と静的オブジェクトは除外

        const targetPos = target.body.position;
        const directionToTarget = new THREE.Vector3(
          targetPos.x - fanPos.x,
          targetPos.y - fanPos.y,
          targetPos.z - fanPos.z
        );

        const distance = directionToTarget.length();

        // 範囲内かチェック
        if (distance < (obj.fanRange || 10)) {
          // 方向ベクトルを正規化
          directionToTarget.normalize();

          // 扇風機の前方にいるかチェック（ドット積）
          const angle = fanDir.dot(directionToTarget);

          if (angle > 0.7) { // 約45度以内
            // 距離による減衰（近いほど強い）
            const forceMagnitude = (obj.fanForce || 5) * (1 - distance / (obj.fanRange || 10));

            // 風の力を適用
            target.body.applyForce(
              new CANNON.Vec3(fanDir.x * forceMagnitude, fanDir.y * forceMagnitude, fanDir.z * forceMagnitude),
  ...
  for (let i = launchingFireworks.length - 1; i >= 0; i--) {
    ...
    if (Math.random() < 0.3) { // 30%の確率で軌跡を残す
      ...
      trailParticle.position.copy(oldPosition);
      trailParticle.userData.life = 20; // 短命
      trailParticle.userData.maxLife = 20;
      ...
      fireworksParticles.push(trailParticle);
    }
  ...
  gameObjects.forEach(obj => {
    if (obj.type === 'train' && obj.orbitAngle !== undefined && obj.orbitSpeed !== undefined) {
      ...
      if (obj.mesh instanceof THREE.Group) {
        ...
        // Y軸のみ回転（進行方向）、X軸とZ軸は0に固定
        group.rotation.x = 0;
        group.rotation.y = headPos.angle;
        group.rotation.z = 0;

        // グループ内の車両グループの回転をリセット（自転を防ぐ）
        group.children.forEach(child => {
          if (child instanceof THREE.Group) {
            // 車両グループ自体の回転を0に（X, Y, Z全て）
            child.rotation.x = 0;
            child.rotation.y = 0;
            child.rotation.z = 0;
          }
        ...
        // 先頭の円錐の回転を再設定（正しい方向を向くように）
        const noseMesh = (group as any).noseMesh as THREE.Mesh | null;
        if (noseMesh) {
          noseMesh.rotation.set(0, 0, -Math.PI / 2); // Z軸周りに-90度回転
        }
      }
